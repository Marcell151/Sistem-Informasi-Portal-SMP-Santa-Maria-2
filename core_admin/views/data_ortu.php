 ...
 * FIX: Penghapusan NIK, Diganti dengan Username Auto-Generate & Nama Wali
 */
...
$sql .= " GROUP BY o.id_ortu ORDER BY o.nama_wali ASC";
                    ...
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        <thead class="bg-white text-xs text-slate-500 uppercase border-b border-[#E2E8F0]">
                            <tr>
                                ...
                                <th class="p-4 font-bold">Username Login</th>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 bg-[#000080]/10 border border-[#000080]/20 text-[#000080] rounded-full flex items-center justify-center flex-shrink-0">
                                ...
                                <td class="p-4 font-mono text-emerald-600 bg-emerald-50/50 font-bold tracking-wider">
                                ...
                                <td class="p-4 text-center text-slate-600 font-bold text-xs">
                                    <?= htmlspecialchars($o['no_hp_ortu'] ?? '-') ?>
                                </td>
                                <td class="p-4 text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        <button onclick="resetPassword(<?= $o['id_ortu'] ?>, '<?= htmlspecialchars($o['username'], ENT_QUOTES) ?>')" 
        ...
        <div class="overflow-y-auto p-6">
            <form action="../actions/tambah_ortu.php" method="POST" id="formTambah" class="space-y-5">
                ...
                <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-100">
                    <label class="<?= $label_class ?> text-[#000080]">Nama Perwakilan Wali Murid *</label>
                    <input type="text" name="nama_wali" required class="<?= $input_class ?>" placeholder="Contoh: Budi Santoso">
                    <p class="text-[10px] text-blue-600 mt-1.5 font-medium flex items-center">
                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>
                        Username login otomatis dibuat dari nama ini (Contoh: budi12)
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2">
                    <div>
                        ...
                        <input type="text" name="nama_ayah" class="<?= $input_class ?>" placeholder="Nama Lengkap Ayah">
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2 border-t border-[#E2E8F0]">
                    <div>
                        ...
                        <input type="text" name="nama_ibu" class="<?= $input_class ?>" placeholder="Nama Lengkap Ibu">
                    </div>
                    <div>
    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl relative z-10 overflow-hidden transform transition-all flex flex-col max-h-[90vh]">
        ...
        <div class="overflow-y-auto p-6">
            <form action="../actions/edit_ortu.php" method="POST" id="formEdit" class="space-y-5">
                ...
                <input type="hidden" name="no_hp_lama" id="edit_no_hp_lama"> <div>
                    <label class="<?= $label_class ?>">Nama Perwakilan Wali Murid *</label>
                    <input type="text" name="nama_wali" id="edit_nama_wali" required class="<?= $input_class ?>">
                </div>
                
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2 border-t border-[#E2E8F0]">
                    <div>
                        <label class="<?= $label_class ?>">Nama Ayah</label>
                        <input type="text" name="nama_ayah" id="edit_nama_ayah" class="<?= $input_class ?>">
                    </div>
                    <div>
                        <label class="<?= $label_class ?>">Pekerjaan Ayah</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2 border-t border-[#E2E8F0]">
                    <div>
                        <label class="<?= $label_class ?>">Nama Ibu</label>
                        <input type="text" name="nama_ibu" id="edit_nama_ibu" class="<?= $input_class ?>">
                    </div>
                    <div>
                        <label class="<?= $label_class ?>">Pekerjaan Ibu</label>
...
function editOrtu(data) {
    ...
    document.getElementById('edit_no_hp_lama').value = data.no_hp_ortu;
